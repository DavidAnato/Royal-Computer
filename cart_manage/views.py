...
def add_to_cart(request, product_id):
    ...
    quantity = request.POST.get('quantity', 1)  # Obtenez la quantité du formulaire, par défaut à 1 si non spécifié
...
def checkout(request):
    ...
    if request.method == 'POST':
        ...
        if shipping_form.is_valid():
            selected_address = shipping_form.cleaned_data['shipping_address']
            # Créez une commande basée sur le contenu du panier et l'adresse sélectionnée
    ...
    context = {
        'cart': cart,
        'items': items,
        'user_addresses': user_addresses,  # Ajoutez les adresses de l'utilisateur au contexte
